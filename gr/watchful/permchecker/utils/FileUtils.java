...
public class FileUtils {
    ...
    //TODO @Watchful11 Merge properties - bearbear12345	
    public static void WriteSettings() {
        Properties prop = new Properties();
        OutputStream output = null;
        try {
            output = new FileOutputStream(mainClass.appdata.getPath());
            // set the properties value
            prop.setProperty("updated_time", mainClass.updatedtime);
            // save properties to project root folder
            prop.store(output, "PermissionsChecker Information File");
        } catch (IOException io) {
            System.out.println("Cannot create application data! Do you have permission to access the folder: " + mainClass.appstore.getPath() + "?");
        } finally {
            if (output != null) {
                try {
                    output.close();
    ...
    public static void ReadSettings() {
        ...
        try {
            input = new FileInputStream(mainClass.appdata.getPath());
            // load a properties file
            prop.load(input);
            // get the property value and print it out
            mainClass.updatedtime = prop.getProperty("updated_time");
